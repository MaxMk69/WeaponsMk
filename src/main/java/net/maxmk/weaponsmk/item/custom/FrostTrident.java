...
public class FrostTrident extends NetheriteTrident{
    public FrostTrident(Properties pProperties) {
        super(pProperties);
    }
                ...
                if (!(f > 0.0F) || player.isInWaterOrRain()) {
                    if (!isTooDamagedToUse(pStack)) {
                        ...
                        if (!pLevel.isClientSide) {
                            ...
                            if (f == 0.0F) {
                                FrostTridentEntity frostTridentEntity = new FrostTridentEntity(pLevel, player, pStack);
                                frostTridentEntity.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 3F, 0.5F);
                                if (player.hasInfiniteMaterials()) {
                                    frostTridentEntity.pickup = AbstractArrow.Pickup.CREATIVE_ONLY;
                                }

                                pLevel.addFreshEntity(frostTridentEntity);
                                pLevel.playSound(null, frostTridentEntity, holder.value(), SoundSource.PLAYERS, 1.0F, 1.0F);
                                if (!player.hasInfiniteMaterials()) {
    ...
    public Projectile asProjectile(Level pLevel, Position pPos, ItemStack pStack, Direction pDirection) {
        FrostTridentEntity frostTridentEntity = new FrostTridentEntity(pLevel, pPos.x(), pPos.y(), pPos.z(), pStack.copyWithCount(1));
        frostTridentEntity.pickup = AbstractArrow.Pickup.ALLOWED;
        return frostTridentEntity;
    }
}
