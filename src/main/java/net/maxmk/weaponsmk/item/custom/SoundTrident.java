...
public class SoundTrident extends NetheriteTrident{
    public SoundTrident(Properties pProperties) {
        super(pProperties);
    }
                    ...
                    if (!isTooDamagedToUse(pStack)) {
                        ...
                        if (!pLevel.isClientSide) {
                            ...
                            if (f == 0.0F) {
                                ...
                                pLevel.addFreshEntity(soundTridentEntity);
                                pLevel.playSound(null, soundTridentEntity, holder.value(), SoundSource.PLAYERS, 1.0F, 1.0F);
                                if (!player.hasInfiniteMaterials()) {
    ...
    public Projectile asProjectile(Level pLevel, Position pPos, ItemStack pStack, Direction pDirection) {
        SoundTridentEntity soundTridentEntity = new SoundTridentEntity(pLevel, pPos.x(), pPos.y(), pPos.z(), pStack.copyWithCount(1));
        soundTridentEntity.pickup = AbstractArrow.Pickup.ALLOWED;
        return soundTridentEntity;
    }
}
