...
public class HolyTrident extends NetheriteTrident{
    public HolyTrident(Properties pProperties) {
        super(pProperties);
    }
                ...
                if (!(f > 0.0F) || player.isInWaterOrRain()) {
                    if (!isTooDamagedToUse(pStack)) {
                        ...
                        if (!pLevel.isClientSide) {
                            ...
                            if (f == 0.0F) {
                                HolyTridentEntity holyTridentEntity = new HolyTridentEntity(pLevel, player, pStack);
                                holyTridentEntity.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 3F, 0.5F);
                                if (player.hasInfiniteMaterials()) {
                                    holyTridentEntity.pickup = AbstractArrow.Pickup.CREATIVE_ONLY;
                                }

                                pLevel.addFreshEntity(holyTridentEntity);
                                pLevel.playSound(null, holyTridentEntity, holder.value(), SoundSource.PLAYERS, 1.0F, 1.0F);
                                if (!player.hasInfiniteMaterials()) {
    ...
    public Projectile asProjectile(Level pLevel, Position pPos, ItemStack pStack, Direction pDirection) {
        HolyTridentEntity holyTridentEntity = new HolyTridentEntity(pLevel, pPos.x(), pPos.y(), pPos.z(), pStack.copyWithCount(1));
        holyTridentEntity.pickup = AbstractArrow.Pickup.ALLOWED;
        return holyTridentEntity;
    }
}
