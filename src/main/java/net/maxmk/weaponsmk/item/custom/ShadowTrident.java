...
public class ShadowTrident extends NetheriteTrident{
    public ShadowTrident(Properties pProperties) {
        super(pProperties);
    }
                ...
                if (!(f > 0.0F) || player.isInWaterOrRain()) {
                    if (!isTooDamagedToUse(pStack)) {
                        ...
                        if (!pLevel.isClientSide) {
                            ...
                            if (f == 0.0F) {
                                ShadowTridentEntity shadowTridentEntity = new ShadowTridentEntity(pLevel, player, pStack);
                                shadowTridentEntity.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 3F, 0.5F);
                                if (player.hasInfiniteMaterials()) {
                                    shadowTridentEntity.pickup = AbstractArrow.Pickup.CREATIVE_ONLY;
                                }

                                pLevel.addFreshEntity(shadowTridentEntity);
                                pLevel.playSound(null, shadowTridentEntity, holder.value(), SoundSource.PLAYERS, 1.0F, 1.0F);
                                if (!player.hasInfiniteMaterials()) {
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        target.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 40, 1)); // 40 ticks = 2 seconds

        // Check light level at target's position
        BlockPos pos = target.blockPosition();
        int lightLevel = level.getMaxLocalRawBrightness(pos); // includes sky & block light

        if (lightLevel <= 6) {
            // Bonus damage in darkness
            target.hurt(level.damageSources().playerAttack((Player) attacker), 3.0F); // extra 3 damage
        }
    ...
    public Projectile asProjectile(Level pLevel, Position pPos, ItemStack pStack, Direction pDirection) {
        ShadowTridentEntity shadowTridentEntity = new ShadowTridentEntity(pLevel, pPos.x(), pPos.y(), pPos.z(), pStack.copyWithCount(1));
        shadowTridentEntity.pickup = AbstractArrow.Pickup.ALLOWED;
        return shadowTridentEntity;
    }
}
